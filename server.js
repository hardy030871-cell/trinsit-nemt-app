...
function seed() {
  ...
  if (!fs.existsSync(FILES.users)) writeJson(FILES.users, DEFAULT_USERS.map(user => ({ ...user, createdAt: new Date().toISOString() })));
  repairDefaultUsers();
...
function isClockedIn(userId) {
  return Boolean(currentAttendance(userId));
}
function requireClockedInForTripWork(req, res) {
  if (req.user.role === 'admin') return true;
  if (isClockedIn(req.user.id)) return true;
  res.status(409).json({ error: 'Clock in before working trips' });
  return false;
}
...
app.post('/api/attendance/admin-adjust', auth, requireRole('admin'), (req, res) => {
  const rows = readJson(FILES.attendance, []);
  const { userId, action, time, note, location } = req.body;
  let row = [...rows].reverse().find(r => r.userId === userId && !r.clockOutAt);
  const users = readJson(FILES.users, []);
  const userIdx = users.findIndex(u => u.id === userId);
  if (action === 'clock_in' && !row) {
    const user = users[userIdx];
    row = { id: uuidv4(), userId, userName: user?.name || 'Unknown', type: 'Hourly', clockInAt: time || new Date().toISOString(), clockOutAt: '', breakStartAt: '', breakEndAt: '', locationIn: location || 'Admin override', locationOut: '', status: 'clocked_in', manualOverride: true, overrideReason: note || '', commissionEntries: [] };
    rows.push(row);
    if (userIdx >= 0) users[userIdx].status = 'available';
  } else if (row && action === 'clock_out') {
    row.clockOutAt = time || new Date().toISOString();
    row.locationOut = location || 'Admin override';
    row.manualOverride = true;
    row.overrideReason = note || '';
    row.status = 'clocked_out';
    if (userIdx >= 0) users[userIdx].status = 'off_duty';
    const live = readJson(FILES.live, {});
    delete live[userId];
    writeJson(FILES.live, live);
  }
...
app.put('/api/attendance/:id', auth, requireRole('admin'), (req, res) => {
  const rows = readJson(FILES.attendance, []);
  const idx = rows.findIndex(row => row.id === req.params.id);
  if (idx < 0) return res.status(404).json({ error: 'Attendance row not found' });
  const allowed = ['type', 'clockInAt', 'clockOutAt', 'breakStartAt', 'breakEndAt', 'locationIn', 'locationOut', 'status', 'overrideReason'];
  for (const key of allowed) {
    if (Object.prototype.hasOwnProperty.call(req.body, key)) rows[idx][key] = req.body[key];
  }
  rows[idx].manualOverride = true;
  writeJson(FILES.attendance, rows);
  res.json(rows[idx]);
});
...
app.post('/api/trips', auth, requireRole('admin','dispatcher','manager'), (req, res) => {
  if (!requireClockedInForTripWork(req, res)) return;
  const trips = readJson(FILES.trips, []);
  const body = req.body;
  const trip = {
    id: nextTripId(trips),
...
app.post('/api/trips/:id/status', auth, requireRole('driver','contractor_driver','admin','dispatcher','manager'), (req, res) => {
  ...
  if (status === 'trip_in_progress' && !requireClockedInForTripWork(req, res)) return;
  const trips = readJson(FILES.trips, []);
...
app.delete('/api/trips/:id', auth, requireRole('admin','dispatcher','manager'), (req, res) => {
  const trips = readJson(FILES.trips, []);
  const trip = trips.find(t => t.id === req.params.id);
  if (!trip) return res.status(404).json({ error: 'Trip not found' });
  if (trip.status !== 'cancelled') return res.status(400).json({ error: 'Only canceled trips can be deleted' });
...
app.post('/api/trip-fields', auth, requireRole('admin'), (req, res) => {
  const settings = readJson(FILES.settings, { payers: [], featureFlags: {}, tripFields: [] });
  settings.tripFields = settings.tripFields || [];
  const label = String(req.body.label || '').trim();
  const type = ['text', 'number', 'date', 'time'].includes(req.body.type) ? req.body.type : 'text';
  if (!label) return res.status(400).json({ error: 'Field label is required' });
  if (settings.tripFields.some(field => field.label.toLowerCase() === label.toLowerCase())) return res.status(400).json({ error: 'Field already exists' });
  settings.tripFields.push({ id: uuidv4(), label, type, required: Boolean(req.body.required) });
  writeJson(FILES.settings, settings);
  res.json(settings);
});

app.put('/api/trip-fields/:id', auth, requireRole('admin'), (req, res) => {
  const settings = readJson(FILES.settings, { payers: [], featureFlags: {}, tripFields: [] });
  settings.tripFields = settings.tripFields || [];
  const idx = settings.tripFields.findIndex(field => field.id === req.params.id);
  if (idx < 0) return res.status(404).json({ error: 'Field not found' });
  const label = String(req.body.label || '').trim();
  const type = ['text', 'number', 'date', 'time'].includes(req.body.type) ? req.body.type : 'text';
  if (!label) return res.status(400).json({ error: 'Field label is required' });
  if (settings.tripFields.some(field => field.id !== req.params.id && field.label.toLowerCase() === label.toLowerCase())) return res.status(400).json({ error: 'Field already exists' });
  settings.tripFields[idx] = { ...settings.tripFields[idx], label, type, required: Boolean(req.body.required) };
  writeJson(FILES.settings, settings);
  res.json(settings);
});

app.delete('/api/trip-fields/:id', auth, requireRole('admin'), (req, res) => {
  const settings = readJson(FILES.settings, { payers: [], featureFlags: {}, tripFields: [] });
  settings.tripFields = (settings.tripFields || []).filter(field => field.id !== req.params.id);
  writeJson(FILES.settings, settings);
  res.json(settings);
});
