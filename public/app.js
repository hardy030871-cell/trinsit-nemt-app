...
function tripStatusActions(trip){
  if(!roleIs('driver','contractor_driver','dispatcher','admin','manager')) return '';
  ...
  const canLeave = !completedDone && !leavingDone && arrivedDone;
...
function renderTrips(){
  ...
  if (form) { form.onsubmit = submitTrip; initTripFormEnhancements(form); }
}
...
function createTripForm(){
  ...
  return `<div class="card"><h3>Create New Trip</h3>
    <form id="tripForm" class="grid two">
      ...
      <input class="long" name="pickupLocation" list="pickupAddressMemory" placeholder="Pickup Address" required>
      <datalist id="pickupAddressMemory"></datalist>
      ...
      <select name="oxygen" id="oxygenSelect" required><option>No</option><option>Yes</option></select>
      <input name="oxygenLiters" id="oxygenLitersInput" placeholder="Oxygen Quantity (Liters)" style="display:none">
      <select name="hasCaregiver" id="caregiverSelect" required><option>No</option><option>Yes</option></select>
      <input name="caregiverCount" id="caregiverCountInput" placeholder="Caregiver Count" style="display:none">
      <select name="hasOtherStop" id="otherStopSelect" required><option>No</option><option>Yes</option></select>
      <input name="otherStop" id="otherStopInput" placeholder="Other Stop Address" style="display:none">
...
function savePickupAddressMemory(value){
  const next = String(value || '').trim();
  if (!next) return;
  const key = 'trinsit_pickup_memory';
  const cur = JSON.parse(localStorage.getItem(key) || '[]');
  const list = [next, ...cur.filter(v => String(v).toLowerCase() !== next.toLowerCase())].slice(0, 25);
  localStorage.setItem(key, JSON.stringify(list));
}

function loadPickupAddressMemory(){
  try { return JSON.parse(localStorage.getItem('trinsit_pickup_memory') || '[]'); } catch { return []; }
}

function initTripFormEnhancements(form){
  const oxygenSelect = form.querySelector('#oxygenSelect');
  const oxygenLitersInput = form.querySelector('#oxygenLitersInput');
  const caregiverSelect = form.querySelector('#caregiverSelect');
  const caregiverCountInput = form.querySelector('#caregiverCountInput');
  const otherStopSelect = form.querySelector('#otherStopSelect');
  const otherStopInput = form.querySelector('#otherStopInput');
  const pickupMemory = form.querySelector('#pickupAddressMemory');

  const addresses = loadPickupAddressMemory();
  if (pickupMemory) pickupMemory.innerHTML = addresses.map(a => '<option value="' + escapeHtml(a) + '"></option>').join('');

  const toggle = () => {
    const oxygenYes = oxygenSelect && oxygenSelect.value === 'Yes';
    if (oxygenLitersInput) {
      oxygenLitersInput.style.display = oxygenYes ? '' : 'none';
      oxygenLitersInput.required = !!oxygenYes;
      if (!oxygenYes) oxygenLitersInput.value = '';
    }

    const caregiverYes = caregiverSelect && caregiverSelect.value === 'Yes';
    if (caregiverCountInput) {
      caregiverCountInput.style.display = caregiverYes ? '' : 'none';
      caregiverCountInput.required = !!caregiverYes;
      if (!caregiverYes) caregiverCountInput.value = '';
    }

    const stopYes = otherStopSelect && otherStopSelect.value === 'Yes';
    if (otherStopInput) {
      otherStopInput.style.display = stopYes ? '' : 'none';
      otherStopInput.required = !!stopYes;
      if (!stopYes) otherStopInput.value = '';
    }
  };

  oxygenSelect && oxygenSelect.addEventListener('change', toggle);
  caregiverSelect && caregiverSelect.addEventListener('change', toggle);
  otherStopSelect && otherStopSelect.addEventListener('change', toggle);
  toggle();
}
async function submitTrip(e){
  ...
  delete payload.hasCaregiver;
  delete payload.hasOtherStop;
