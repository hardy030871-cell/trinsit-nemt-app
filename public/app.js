...
function renderLogin(){
  app.innerHTML = `<div class="login-wrap premium"><div class="login-card premium-card"><img class="login-logo" src="/logo.png" alt="TRINSIT"/><div class="eyebrow">TRINSIT Secure Access</div><h1>Enter your passcode</h1><p class="muted">Use your 6 digits plus special symbol. Example format: 001900!</p><form id="loginForm" class="grid"><label>Passcode<input name="passcode" type="password" inputmode="text" placeholder="001900!" required></label><button>Unlock App</button></form><div class="small muted center">No email or password required.</div></div></div>`;
...
function usersView(){ if(!roleIs('admin')) return '<div class="panel">Admin only.</div>'; return `<div class="split responsive-top"><section class="panel"><h3>Create User</h3><form id="userForm" class="grid"><label>Name<input name="name" required></label><label>Email (optional)<input name="email" type="email"></label><label>Login Code<input name="loginCode" required placeholder="123456!"></label><label>Role<select name="role"><option>manager</option><option>dispatcher</option><option>driver</option><option>contractor_driver</option><option>admin</option></select></label><button>Create</button></form></section><section class="panel"><h3>Manage Users</h3>${state.data.users.map(u=>`<div class="item"><strong>${escapeHtml(u.name)}</strong><div class="small muted">${escapeHtml(u.role)} • ${escapeHtml(u.accountStatus||'active')}</div><div class="actions"><button class="smallbtn" onclick="resetPin('${u.id}')">Reset Login Code</button><button class="smallbtn" onclick="setStatus('${u.id}','suspended')">Suspend</button><button class="smallbtn" onclick="setStatus('${u.id}','closed')">Close</button><button class="smallbtn danger" onclick="setStatus('${u.id}','deleted')">Delete</button></div></div>`).join('')}</section></div>`; }
