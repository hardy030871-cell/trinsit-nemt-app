...
function canWorkTrips(){ return roleIs('admin') || Boolean(currentAttendance(state.user?.id)); }
...
function tripCard(trip){
  return `<div class="trip-card">
    ...
    <div class="actions"><button onclick="openTrip('${trip.id}')">Open</button>${trip.status === 'cancelled' && roleIs('admin','dispatcher','manager') ? `<button class="ghost danger" onclick="deleteCanceledTrip('${trip.id}')">Delete</button>` : ''}</div>
  </div>`;
}
...
function openTrip(id){
  ...
  modal.innerHTML = `<div class="modal-card"><div class="row between"><h3>${trip.id}</h3><button class="ghost" onclick="this.closest('.modal').remove()">✕</button></div>
    <div class="stack">
      ...
      <div><strong>Oxygen:</strong> ${escapeHtml(trip.oxygen)} ${trip.otherStop ? `| <strong>Stop:</strong> ${escapeHtml(trip.otherStop)}` : ''}</div>
      ${tripCustomFieldRows(trip)}
...
function tripCustomFieldRows(trip){
  const fields = state.settings.tripFields || [];
  if (!fields.length) return '';
  const custom = trip.customFields || {};
  return fields.map(field => custom[field.id] ? `<div><strong>${escapeHtml(field.label)}:</strong> ${escapeHtml(custom[field.id])}</div>` : '').join('');
}

function tripAdminActions(trip){
  if (!roleIs('admin','dispatcher','manager')) return '';
  const cancelBtn = trip.status !== 'completed' && trip.status !== 'cancelled' ? `<button class="ghost danger" onclick="advanceTrip('${trip.id}','cancelled')">Cancel Trip</button>` : '';
  const deleteBtn = trip.status === 'cancelled' ? `<button class="ghost danger" onclick="deleteCanceledTrip('${trip.id}')">Delete Canceled Trip</button>` : '';
  return cancelBtn || deleteBtn ? `<div class="actions">${cancelBtn}${deleteBtn}</div>` : '';
}
...
function tripStatusActions(trip){
  ...
  const clockedIn = canWorkTrips();
  return `<div class="progress-vertical">
    ${clockedIn ? '' : `<div class="muted">Clock in before starting a trip.</div>`}
    <button class="progress-step-btn ${inProgressDone?'done':''}" ${canStart && clockedIn?'':'disabled'} onclick="advanceTrip('${trip.id}','trip_in_progress')">Trip In Progress</button>
...
async function deleteCanceledTrip(tripId){
  if (!confirm('Delete this canceled trip?')) return;
  try {
    await api(`/api/trips/${tripId}`, { method:'DELETE' });
    toast('Canceled trip deleted');
...
function createTripForm(){
  ...
  return `<div class="card"><h3>Create New Trip</h3>
    ${clockedIn ? '' : `<div class="pill">Clock in before creating or assigning trips.</div>`}
    <form id="tripForm" class="grid two">
      ...
      <div class="full"><button ${clockedIn?'':'disabled'}>Create Trip</button></div>
    </form></div>`;
}
function customTripFieldInputs(){
  return (state.settings.tripFields || []).map(field => `<input name="custom_${field.id}" type="${escapeHtml(field.type || 'text')}" placeholder="${escapeHtml(field.label)}" ${field.required?'required':''}>`).join('');
}
...
async function submitTrip(e){
  ...
  (state.settings.tripFields || []).forEach(field => {
    payload.customFields[field.id] = fd.get(`custom_${field.id}`) || '';
    delete payload[`custom_${field.id}`];
  });
...
function renderDispatcher(){
  const clockedIn = canWorkTrips();
  document.getElementById('page').innerHTML = `<div class="card"><h3>Dispatcher Board</h3>${clockedIn ? '' : `<div class="pill">Clock in before assigning trips.</div>`}<div class="board-grid">${state.trips.map(t=>`
    ...
    <div class="row"><label>Drivers<select multiple ${clockedIn?'':'disabled'} onchange="assignDriver('${t.id}', this)">${state.users.filter(u=>['driver','contractor_driver'].includes(u.role)).map(u=>`<option value="${u.id}" ${(t.driverIds||[]).includes(u.id)?'selected':''}>${escapeHtml(u.name)}</option>`).join('')}</select></label></div>
  ...
  document.getElementById('page').innerHTML = `<div class="grid two">
    ...
    ${roleIs('admin') ? `<div class="card"><h3>Admin Manual Attendance</h3>
      <form id="manualAttendance" class="stack">
        ...
        <input name="location" placeholder="Address / location">
        <input name="note" placeholder="Reason" required>
        <button>Save</button>
      </form></div>` : ''}
  </div>
  <div class="card"><h3>Attendance Log</h3><div class="list">${state.attendance.slice().reverse().map(a=>`<div class="list-row"><span><strong>${escapeHtml(a.userName)}</strong><br><span class="muted">${fmtDateTime(a.clockInAt)} ${a.clockOutAt?`→ ${fmtDateTime(a.clockOutAt)}`:''}</span><br><span class="muted">In: ${escapeHtml(a.locationIn || 'No address')} ${a.locationOut?`| Out: ${escapeHtml(a.locationOut)}`:''}</span></span><span>${a.manualOverride?'Manual':''}${roleIs('admin')?` <button class="ghost" onclick="editAttendance('${a.id}')">Edit</button>`:''}</span></div>`).join('')}</div></div>`;
...
async function clockOut(){ try{ const location = await getCurrentAddress(); await api('/api/attendance/clock-out', { method:'POST', body: JSON.stringify({ location })}); toast('Clocked out'); await refreshData(); }catch(err){ toast(err.message);} }

async function editAttendance(id){
  const row = state.attendance.find(a => a.id === id);
  if (!row) return;
  const clockInAt = prompt('Clock in time', row.clockInAt || '');
  if (clockInAt === null) return;
  const clockOutAt = prompt('Clock out time', row.clockOutAt || '');
  if (clockOutAt === null) return;
  const locationIn = prompt('Clock in address', row.locationIn || '');
  if (locationIn === null) return;
  const locationOut = prompt('Clock out address', row.locationOut || '');
  if (locationOut === null) return;
  const status = clockOutAt ? 'clocked_out' : 'clocked_in';
  try {
    await api(`/api/attendance/${id}`, { method:'PUT', body: JSON.stringify({ clockInAt, clockOutAt, locationIn, locationOut, status }) });
...
function renderControl(){
  document.getElementById('page').innerHTML = `<div class="grid two">
    ...
    <div class="card"><h3>Add Trip Field</h3><form id="tripFieldForm" class="stack"><input name="label" placeholder="Field label" required><select name="type"><option value="text">Text</option><option value="number">Number</option><option value="date">Date</option><option value="time">Time</option></select><label class="check"><input type="checkbox" name="required"> Required field</label><button>Save Field</button></form></div>
    <div class="card"><h3>Trip Form Fields</h3><div class="list">${(state.settings.tripFields||[]).map(field=>`<div class="list-row"><span>${escapeHtml(field.label)} · ${escapeHtml(field.type || 'text')} · ${field.required?'Required':'Optional'}</span><span><button class="ghost" onclick="editTripField('${field.id}')">Edit</button><button class="ghost danger" onclick="deleteTripField('${field.id}')">Delete</button></span></div>`).join('')}</div></div>
  ...
  document.getElementById('tripFieldForm').onsubmit = async e => {
    e.preventDefault();
    const fd = new FormData(e.target);
    try {
      await api('/api/trip-fields', { method:'POST', body: JSON.stringify({ label: fd.get('label'), type: fd.get('type'), required: fd.get('required') === 'on' }) });
      toast('Trip field saved');
      await refreshData();
    } catch(err){ toast(err.message); }
  };
}
...
async function editTripField(id){
  const field = (state.settings.tripFields || []).find(item => item.id === id);
  if (!field) return;
  const label = prompt('Field label', field.label);
  if (label === null) return;
  const type = prompt('Field type: text, number, date, time', field.type || 'text') || 'text';
  const required = confirm('Make this field required?');
  try {
    await api(`/api/trip-fields/${id}`, { method:'PUT', body: JSON.stringify({ label, type, required }) });
    toast('Trip field updated');
    await refreshData();
  } catch(err){ toast(err.message); }
}
async function deleteTripField(id){
  if (!confirm('Delete this trip form field? Existing trip values will remain hidden unless you add the field again.')) return;
  try {
    await api(`/api/trip-fields/${id}`, { method:'DELETE' });
    toast('Trip field deleted');
    await refreshData();
  } catch(err){ toast(err.message); }
}
...
function getPosition(){
  return new Promise(resolve => {
    ...
    navigator.geolocation.getCurrentPosition(pos => resolve(pos), () => resolve(null), { enableHighAccuracy: true, maximumAge: 10000, timeout: 10000 });
  });
}
