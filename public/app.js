...
async function loadAll(){ await Promise.all([loadTrips(),loadUsers(),loadPayers(),loadAttendance(),loadExpenses(),loadInspections(),loadIncidents(),loadEquipment(),loadMessages(),loadChatUsers(),loadDirectory(),loadChannels(),loadGps(),loadPriceSettings(),loadDashboardSummary()]); state.unreadMessages = computeUnreadMessages(); }
...
function render(){
  ...
  <div class="shell">
    ...
    <main class="main">
      <header class="topbar"><h2>${pageTitle()}</h2><div class="top-actions"><button class="notif-btn" onclick="openNotifications()">🔔 <span id="notifBadge" class="notif-pill">${state.data.notifications.filter(x=>!x.read).length}</span></button><button class="notif-btn" onclick="goRoute('chat')">💬 <span id="msgBadge" class="notif-pill">${state.unreadMessages||0}</span></button></div></header>
      ${routeView()}
    </main>
  </div>`;
  bindPage();
...
function pageTitle(){ return {dashboard: roleIs('driver','contractor_driver') ? 'Driver Home' : 'Admin Dashboard', trips:'Trips', attendance:'Attendance', map:'Live Map', expenses:'Expenses', inspection:'Vehicle Inspection', incidents:'Incident Report', equipment:'Equipment', chat:'Team Chat', commission:'Commission Entry', pricing:'Pricing Admin', users:'User Admin'}[state.route] || 'TRINSIT'; }
function goRoute(route){ state.route = route; render(); }
window.goRoute = goRoute; window.logout = logout; window.quickClock = quickClock; window.selectChatTarget = function(target){ state.selectedChatTarget = target; state.unreadMessages = computeUnreadMessages(); render(); }; window.openNotifications = function(){ state.route='dashboard'; state.data.notifications = state.data.notifications.map(n=>({...n,read:true})); render(); };
...
function workSelectionView(){
  const canCommission = !!state.user?.showCommissionPage;
...
function bindPage(){
  ...
  if(tripForm){
    ...
    syncTripFields();
    bindAddressSuggest('pickupLocationInput', 'pickupSuggestions');
    bindAddressSuggest('dropoffLocationInput', 'dropoffSuggestions');
    ...
    const mileageInput = tripForm.querySelector('[name="googleMileage"]');
    let autoMileageTimer;
    const updateMileageFromAddresses = ()=>{
      clearTimeout(autoMileageTimer);
      autoMileageTimer = setTimeout(async ()=>{
        const from = pickupInput?.value?.trim();
        const to = dropoffInput?.value?.trim();
        if(!from || !to || !mileageInput) return;
        try{
          const out = await apiJson(`/api/geo/mileage?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`);
          if(out && typeof out.miles === 'number' && out.miles > 0) mileageInput.value = Number(out.miles).toFixed(1);
        }catch{}
      }, 400);
    };
    pickupInput && pickupInput.addEventListener('change', updateMileageFromAddresses);
    pickupInput && pickupInput.addEventListener('blur', updateMileageFromAddresses);
    dropoffInput && dropoffInput.addEventListener('change', updateMileageFromAddresses);
    dropoffInput && dropoffInput.addEventListener('blur', updateMileageFromAddresses);
    tripForm.onsubmit = async (e)=>{
      ...
      if((!obj.googleMileage || Number(obj.googleMileage) <= 0) && obj.pickupLocation && obj.dropoffLocation){
        try{
          const out = await apiJson(`/api/geo/mileage?from=${encodeURIComponent(obj.pickupLocation)}&to=${encodeURIComponent(obj.dropoffLocation)}`);
          if(out && typeof out.miles === 'number' && out.miles > 0) obj.googleMileage = Number(out.miles).toFixed(1);
        }catch{}
      }
      obj.mileage = Number(obj.googleMileage || 0);
      try{ await apiJson('/api/trips','POST',obj); await loadTrips(); await loadDashboardSummary(); render(); toast('Information entered successfully'); toast('Trip created successfully'); }catch(err){ toast('Trip', err.message); }
    };
  ...
  if(clockInForm){
    clockInForm.onsubmit = async (e)=>{
      e.preventDefault();
      if(!confirm('Status: Clock In. Do you want to proceed with this action?')) return;
      try{
        ...
        toast('Information entered successfully');
        toast('Clock in saved successfully');
