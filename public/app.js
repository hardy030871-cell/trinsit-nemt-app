const state = {
  ...
  markers: {},
  autoGpsWatchId: null,
  data: { trips: [], users: [], payers: [], attendance: [], expenses: [], inspections: [], incidents: [], equipment: [], notifications: [], messages: [], gps: [], priceSettings: {}, dashboardSummary: null },
};
const app = document.getElementById('app');
const tripStepOrder = ['assigned','received','trip_in_progress','arrived','leaving_with_patient','drop_off','completed'];
...
async function quickClock(type){
  try{
    ...
    if(type==='clock_in'){
      const commissionPay = confirm('Commission pay for this shift?');
      payload.commissionPay = commissionPay;
      if(commissionPay){
        payload.commissionTrips = [{ patientName:'', pickupLocation:'', date:'' }];
      }
    }
    await apiJson('/api/attendance/clock','POST',payload);
    if(type==='clock_in' || type==='lunch_in') startAutoGps();
...
async function loadAttendance(){ if(roleIs('admin','manager','dispatcher')) state.data.attendance = await apiJson('/api/attendance'); else state.data.attendance=[]; }
...
async function loadAll(){ await Promise.all([loadTrips(),loadUsers(),loadPayers(),loadAttendance(),loadExpenses(),loadInspections(),loadIncidents(),loadEquipment(),loadMessages(),loadGps(),loadPriceSettings(),loadDashboardSummary()]); }
...
function render(){
  ...
  if(roleIs('admin')) nav.push(['pricing','Pricing'], ['users','Users']);
  ...
  <div class="shell">
    <aside class="sidebar">
      ...
      <div class="userbox"><strong>${escapeHtml(state.user.name)}</strong><div class="muted small">${escapeHtml(state.user.email)} • ${escapeHtml(state.user.role)}</div></div>
...
function renderLogin(){
  app.innerHTML = `<div class="login-wrap"><div class="login-card"><img class="login-logo" src="/logo.png" alt="TRINSIT"/><h1>TRINSIT</h1><p class="muted">Secure operations login</p><form id="loginForm" class="grid"><label>Email<input name="email" type="email" required></label><label>Password<input name="password" type="password"></label><div class="or">or login with PIN</div><label>6-digit PIN<input name="pin" inputmode="numeric" pattern="[0-9]{6}" maxlength="6"></label><button>Login</button></form></div></div>`;
  document.getElementById('loginForm').onsubmit = async (e)=>{
    e.preventDefault();
    const fd = new FormData(e.target); const body = Object.fromEntries(fd.entries());
    if(!body.password) delete body.password; if(!body.pin) delete body.pin;
...
function tripTable(trips){
  return `<div class="table-wrap"><table><thead><tr><th>Trip ID</th><th>Date</th><th>Patient</th><th>Pickup</th><th>Dropoff</th><th>Service</th><th>Drivers</th><th>Status</th>${roleIs('admin','manager','dispatcher')?'<th>Payer</th><th>Mileage</th><th>Gross</th>':''}<th>Actions</th></tr></thead><tbody>${trips.map(t=>{
    ...
    return `<tr><td>${escapeHtml(t.tripNumber||t.id)}</td><td>${escapeHtml(t.pickupDate)} ${escapeHtml(t.pickupTime||'')}</td><td>${escapeHtml(t.patientName)}</td><td>${escapeHtml(t.pickupLocation)}</td><td>${escapeHtml(t.dropoffLocation)}</td><td>${escapeHtml(t.service)}</td><td>${escapeHtml(drivers)}</td><td>${escapeHtml(t.status)}</td>${roleIs('admin','manager','dispatcher')?`<td>${escapeHtml(t.payer||'')}</td><td>${Number(t.googleMileage||t.mileage||0).toFixed(1)}</td><td>${money(t.priceBreakdown?.total||0)}</td>`:''}<td><button class="smallbtn" onclick="showTrip('${t.id}')">Open</button></td></tr>`;
  }).join('')}</tbody></table></div>`;
}
function tripStatusActions(trip){
  if(!roleIs('driver','contractor_driver')) return '';
  const idx = tripStepOrder.indexOf(trip.status); const next = tripStepOrder[idx+1];
  let html = '';
  if(next){ html += `<button onclick="advanceTrip('${trip.id}','${next}')">${next.replace(/_/g,' ')}</button>`; }
  if(trip.status==='arrived'){ html += `<label class="upload-inline">Upload Facesheet<input type="file" accept="image/*,.pdf" onchange="uploadFacesheet('${trip.id}', this.files[0])"></label>`; }
  return html;
}
function tripsView(){
...
function attendanceView(){
  const rows = state.data.attendance.map(a=>`<tr><td>${escapeHtml(a.name)}</td><td>${escapeHtml(a.type)}</td><td>${fmtDate(a.createdAt)}</td><td>${escapeHtml(a.address||'')}</td><td>${a.commissionPay?'Yes':'No'}</td><td>${(a.commissionTrips||[]).map(t=>`${escapeHtml(t.patientName)} / ${escapeHtml(t.pickupLocation)} / ${escapeHtml(t.date)}`).join('<br>')}</td>${roleIs('admin')?`<td><button class="smallbtn" onclick="editAttendance('${a.id}')">Edit</button></td>`:''}</tr>`).join('');
  return `<div class="panel"><h3>Attendance Log</h3><div class="muted small">Clock in starts live GPS permission for dispatch view until clock out.</div><div class="table-wrap"><table><thead><tr><th>User</th><th>Type</th><th>Time</th><th>Address</th><th>Commission Pay</th><th>Commission Entries</th>${roleIs('admin')?'<th>Edit</th>':''}</tr></thead><tbody>${rows}</tbody></table></div></div>`;
}
function mapView(){ return `<section class="map-full"><div id="liveMap"></div></section>`; }
...
function chatView(){
  const options = state.data.users.filter(u=>u.id!==state.user.id && u.accountStatus==='active').map(u=>`<option value="${u.id}">${escapeHtml(u.name)} (${u.role})</option>`).join('');
  return `<div class="split responsive-top"><section class="panel"><h3>Send Message</h3><form id="chatForm" class="grid"><label>Choose one or multiple users<select name="recipientIds" id="recipientIds" multiple size="7">${options}</select></label><label>Message<textarea name="text" required></textarea></label><button>Send</button></form></section><section class="panel"><h3>Conversation</h3><div class="chat-feed">${state.data.messages.slice().reverse().map(m=>`<div class="chatmsg"><strong>${escapeHtml(m.userName)}</strong><div>${escapeHtml(m.text)}</div><div class="small muted">${fmtDate(m.createdAt)} ${m.recipientIds?.length?`• to ${m.recipientIds.map(id=>state.data.users.find(u=>u.id===id)?.name||'').join(', ')}`:'• broadcast'}</div></div>`).join('')}</div></section></div>`;
}
function pricingView(){ if(!roleIs('admin')) return '<div class="panel">Admin only.</div>'; const s=state.data.priceSettings; return `<div class="split responsive-top"><section class="panel"><h3>Pricing</h3><form id="pricingForm" class="grid trip-grid"><label>Ambulatory<input name="ambulatory" type="number" value="${s.services?.ambulatory||0}"></label><label>Wheelchair<input name="wheelchair" type="number" value="${s.services?.wheelchair||0}"></label><label>Stretcher<input name="stretcher" type="number" value="${s.services?.stretcher||0}"></label><label>Stair Chair<input name="climbing_stairs_chair" type="number" value="${s.services?.climbing_stairs_chair||0}"></label><label>Own Wheelchair<input name="own_wheelchair" type="number" value="${s.services?.own_wheelchair||0}"></label><label>Weight Threshold<input name="bariatricThreshold" type="number" value="${s.bariatricThreshold||250}"></label><label>Weight Surcharge<input name="weightSurcharge" type="number" value="${s.weightSurcharge||0}"></label><label>Oxygen Base<input name="oxygenBase" type="number" value="${s.oxygen?.base||0}"></label><label>Oxygen Per Liter<input name="oxygenPerLiter" type="number" value="${s.oxygen?.perLiter||0}"></label><label>Extra Stop<input name="extraStop" type="number" value="${s.extraStop||0}"></label><label>11-39 Mile Rate<input name="tier2" type="number" value="${s.mileageTiers?.[1]?.rate||0}"></label><label>40-99 Mile Rate<input name="tier3" type="number" value="${s.mileageTiers?.[2]?.rate||0}"></label><label>100+ Mile Rate<input name="tier4" type="number" value="${s.mileageTiers?.[3]?.rate||0}"></label><button>Save</button></form></section><section class="panel"><h3>Payers</h3><form id="payerForm" class="inline-form"><input name="name" placeholder="Add new payer"><button>Add</button></form>${state.data.payers.map(p=>`<div class="item">${escapeHtml(p)}</div>`).join('')}</section></div>`; }
function usersView(){ if(!roleIs('admin')) return '<div class="panel">Admin only.</div>'; return `<div class="split responsive-top"><section class="panel"><h3>Create User</h3><form id="userForm" class="grid"><label>Name<input name="name" required></label><label>Email<input name="email" type="email" required></label><label>Password<input name="password" required></label><label>6-digit PIN<input name="pin" pattern="[0-9]{6}" maxlength="6" required></label><label>Role<select name="role"><option>manager</option><option>dispatcher</option><option>driver</option><option>contractor_driver</option><option>admin</option></select></label><button>Create</button></form></section><section class="panel"><h3>Manage Users</h3>${state.data.users.map(u=>`<div class="item"><strong>${escapeHtml(u.name)}</strong><div class="small muted">${escapeHtml(u.email)} • ${escapeHtml(u.role)} • ${escapeHtml(u.accountStatus||'active')}</div><div class="actions"><button class="smallbtn" onclick="resetPin('${u.id}')">Reset PIN</button><button class="smallbtn" onclick="setStatus('${u.id}','suspended')">Suspend</button><button class="smallbtn" onclick="setStatus('${u.id}','closed')">Close</button><button class="smallbtn danger" onclick="setStatus('${u.id}','deleted')">Delete</button></div></div>`).join('')}</section></div>`; }

function bindPage(){
  const tripForm = document.getElementById('tripForm');
  if(tripForm){
    const toggleConditional = ()=>{
      const oxygenYes = document.getElementById('oxygenSelect').value==='Yes';
      const caregiverYes = document.getElementById('caregiverSelect').value==='Yes';
      document.querySelector('#oxygenLitersWrap input').required = oxygenYes; document.getElementById('oxygenLitersWrap').style.display = oxygenYes ? 'block':'none';
      document.querySelector('#caregiverCountWrap input').required = caregiverYes; document.getElementById('caregiverCountWrap').style.display = caregiverYes ? 'block':'none';
    };
    document.getElementById('oxygenSelect').onchange = toggleConditional; document.getElementById('caregiverSelect').onchange = toggleConditional; toggleConditional();
    document.getElementById('hasStop').onchange = (e)=>{ document.getElementById('stopWrap').innerHTML = e.target.value==='Yes' ? `<label>Additional Stop Address<input name="stop1" required></label>` : ''; };
    tripForm.onsubmit = async (e)=>{
      e.preventDefault();
      const fd = new FormData(tripForm); const obj = Object.fromEntries(fd.entries());
      obj.additionalStops = [obj.stop1].filter(Boolean); delete obj.stop1;
      obj.oxygen = obj.oxygen === 'Yes';
      obj.assignedDriverIds = [obj.driver1, obj.driver2].filter(Boolean); delete obj.driver1; delete obj.driver2;
  ...
  if(expenseForm){
    const setExpenseFields=()=>{ const cat=document.getElementById('expenseCategory').value; document.getElementById('expenseNoteWrap').style.display = cat==='Maintenance'?'block':'none'; document.querySelector('#expenseNoteWrap textarea').required = cat==='Maintenance'; document.getElementById('expenseOtherWrap').style.display = cat==='Other'?'block':'none'; document.querySelector('#expenseOtherWrap input').required = cat==='Other'; };
  ...
  const userForm = document.getElementById('userForm'); if(userForm) userForm.onsubmit = async e=>{ e.preventDefault(); try{ const out = await apiJson('/api/users','POST',Object.fromEntries(new FormData(userForm).entries())); await loadUsers(); render(); toast('User created', `PIN ${out.plainPin || ''}`); }catch(err){ toast('User', err.message); } };
}
...
function renderMap(refreshOnly){
  ...
  state.data.gps.forEach(g=>{
    seen.add(g.userId);
    if(state.markers[g.userId]) state.markers[g.userId].setLatLng([g.lat,g.lng]).bindPopup(`<strong>${escapeHtml(g.name)}</strong><br>${escapeHtml(g.role)}<br>${fmtDate(g.updatedAt)}`);
    else state.markers[g.userId] = L.marker([g.lat,g.lng]).addTo(state.map).bindPopup(`<strong>${escapeHtml(g.name)}</strong><br>${escapeHtml(g.role)}`);
  });
  Object.keys(state.markers).forEach(id=>{ if(!seen.has(id)){ state.map.removeLayer(state.markers[id]); delete state.markers[id]; } });
...
window.resetPin = async id=>{ const pin = prompt('Enter new 6-digit PIN'); if(!pin) return; try{ await apiJson(`/api/users/${id}`,'PUT',{resetPin:pin}); await loadUsers(); toast('PIN reset'); }catch(err){ toast('PIN', err.message); } };
window.editAttendance = async id=>{ const address = prompt('New address'); if(address==null) return; try{ await apiJson(`/api/attendance/${id}`,'PUT',{address}); await loadAttendance(); render(); }catch(err){ toast('Attendance', err.message); } };
