...
function attendanceView(){
  ...
  const clockedIn = !!latestSelf && ['clock_in','lunch_in'].includes(latestSelf.type);
  ...
  return `<div class="split responsive-top"><section class="panel"><h3>${clockedIn ? 'You are clocked in' : 'Clock In'}</h3><div class="muted small">Clock in starts live GPS permission for dispatch view until clock out. Clocked-in users are also added to Team Chat and shown on the Live Map.</div>${clockedIn ? `<div class="item"><strong>Status:</strong> ${escapeHtml(latestSelf.type)}<div class="small muted">${escapeHtml(latestSelf.address||'')}</div></div>` : `<form id="clockInForm" class="grid trip-grid compact"><label>Pay Type<select name="payType" id="payTypeSelect"><option value="hourly">Hourly</option><option value="per_trip">Per Trip</option><option value="commission">Commission</option></select></label><div id="commissionWrap" class="full hidden"><div class="sectionhead"><strong>Commission Entries</strong><button type="button" class="smallbtn" id="addCommissionRow">Add Entry</button></div><div id="commissionRows"></div></div><div class="full actions"><button>Clock In Now</button></div></form>`}</section><section class="panel"><h3>Attendance Log</h3><div class="table-wrap"><table><thead><tr><th>User</th><th>Type</th><th>Time</th><th>Address</th><th>Pay Type</th><th>Commission Entries</th>${roleIs('admin')?'<th>Edit</th>':''}</tr></thead><tbody>${rows}</tbody></table></div></section></div>`;
}
...
function bindPage(){
  ...
  const docDownloadForm = document.getElementById('docDownloadForm'); if(docDownloadForm) docDownloadForm.onsubmit = async e=>{ e.preventDefault(); const date = new FormData(docDownloadForm).get('date'); const token = state.token; const res = await fetch(`${API_BASE}/api/uploads/daily-download?date=${encodeURIComponent(date)}`, { headers: { Authorization: `Bearer ${token}` } }); if(!res.ok){ try{ const err = await res.json(); throw new Error(err.error || 'Download failed'); }catch(err){ toast('Documents', err.message); return; } } const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `trinsit-uploads-${date}.zip`; document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(url); toast('Documents', 'Download started'); };
}
