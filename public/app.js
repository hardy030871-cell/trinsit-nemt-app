...
function logout(){ localStorage.removeItem('trinsit_token'); state.token=''; location.reload(); }
...
function renderMap(refreshOnly=false){
  ...
  if (!refreshOnly) page.innerHTML = `<div class="card map-card"><h3>Live Map</h3><div id="liveMap"></div></div>`;
  ...
  points.forEach(p => {
    const marker = L.marker([p.lat, p.lng]).addTo(state.map).bindPopup(`${escapeHtml(p.name)} · ${escapeHtml(p.role)}`);
    state.markers[p.userId] = marker;
  });
  setTimeout(()=> state.map.invalidateSize(), 200);
}
setInterval(() => { if (state.page === 'live-map') refreshData(); }, 12000);
...
function renderChat(){
  ...
  document.getElementById('page').innerHTML = `<div class="grid two"><div class="card"><h3>Contacts</h3><input id="contactFilter" placeholder="Search users"><div class="list">${contacts.map(u=>`<label class="check"><input type="checkbox" value="${u.id}" class="chat-user-check"> ${escapeHtml(u.name)} <span class="muted">${u.status||''}</span></label>`).join('')}</div></div>
...
function renderEquipment(){
  document.getElementById('page').innerHTML = `<div class="card"><h3>Equipment Inventory</h3><div class="list">${state.equipment.map(i=>`<div class="list-row"><span>${escapeHtml(i.name)}</span><span>${i.required?'Required':'Optional'}</span></div>`).join('')}</div></div>`;
}
...
function renderControl(){
  document.getElementById('page').innerHTML = `<div class="card"><h3>Admin Control Panel</h3><div class="stack"><div class="muted">Use this page to control users, app features, and permissions. This rebuild keeps the panel simple while stabilizing the app foundation.</div><div class="grid two"><div class="card inset"><strong>Payers</strong><div>${state.settings.payers.map(p=>`<div>${escapeHtml(p)}</div>`).join('')}</div></div><div class="card inset"><strong>Feature Flags</strong>${Object.entries(state.settings.featureFlags||{}).map(([k,v])=>`<div>${escapeHtml(k)}: ${v?'On':'Off'}</div>`).join('')}</div></div></div></div>`;
}

async function sendLiveLocation(){
  if (!navigator.geolocation || !currentAttendance(state.user.id)) return;
